        ...
        if cloudtype == 'moodle':
            ...
            if loged:
                ...
                for f in files:
                    ...
                    while resp is None:
                          if user_info['uploadtype'] == 'evidence':
                             fileid,resp = client.upload_file(f,evidence,fileid,progressfunc=uploadFile,args=(bot,message,originalfile,thread),token=token)
                          if user_info['uploadtype'] == 'draft':
                             fileid,resp = client.upload_file_draft(f,progressfunc=uploadFile,args=(bot,message,originalfile,thread),token=token)
                             draftlist.append(resp)
                          if user_info['uploadtype'] == 'perfil':
                             fileid,resp = client.upload_file_perfil(f,progressfunc=uploadFile,args=(bot,message,originalfile,thread),token=token)
                             draftlist.append(resp)
                          if user_info['uploadtype'] == 'blog':
                             fileid,resp = client.upload_file_blog(f,progressfunc=uploadFile,args=(bot,message,originalfile,thread),token=token)
                             draftlist.append(resp)
                          if user_info['uploadtype'] == 'calendar':
                             fileid,resp = client.upload_file_calendar(f,progressfunc=uploadFile,args=(bot,message,originalfile,thread),token=token)
                             draftlist.append(resp)
                          iter += 1
                          if iter>=10:
                              break
...
def ddl(update,bot,message,url,file_name='',thread=None,jdb=None):
    ...
    if not downloader.stoping:
        if file:
            ...
            bot.editMessageText(message,'❌Enlace No Procesado❌')
...
def onmessage(update,bot:ObigramClient):
    try:
        ...
        if '/start' in msgText:
            start_msg= 'Welcome!\n'
            start_msg+= 'Para saber como usar /tuto'
...
def main():
    ...
    bot_token = '5345943639:AAGCfrKxL8Y28rIecpOxOgbY1XP1RW-n_MI'
